...
function stateSnapshot(){
 ...
 const run=safeRun(operator.run),telemetry=tailRows(run,'telemetry.jsonl',120),decisions=tailRows(run,'model-decisions.jsonl',40).map(compactDecision),actions=tailRows(run,'agent-actions.jsonl',100),events=tailRows(run,'events.jsonl',50),recordedMessages=tailRows(run,'messages.jsonl',50);
