...
Thrust lever position is not engine output. `engines.actualThrottleRatio` reports the simulator's resulting throttle demand and `engines.n1Percent` reports engine spool speed, both in engine order. `engines.fadecMode` uses 0=outside detent range, 1=CLB/cruise, 2=MCT/reduced takeoff, 3=TOGA. Mode 3 alone does not identify why TOGA is active. In this installation, a low-speed event can leave TOGA demand active even with levers at idle. The native `autothrust_disconnect` control disengages and disarms autothrust; `autothrust` can subsequently re-arm and engage it. Protection can reactivate if the low-speed condition persists. Do not treat disconnection as a substitute for restoring a safe flight state, or assume an accepted lever command has reduced thrust. Monitor engine response, airspeed and guidance.
